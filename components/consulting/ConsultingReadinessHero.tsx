...
import type { ConsultingReadinessScore } from "@/lib/consulting-intelligence";
import { cn } from "@/lib/utils";

type ConsultingReadinessHeroProps = {
  readiness: ConsultingReadinessScore;
};

const labelTone: Record<ConsultingReadinessScore["label"], string> = {
  not_ready: "text-muted-foreground",
  incomplete: "text-[var(--warning-text)]",
  pre_check: "text-accent",
  review_ready: "text-[var(--success-text)]",
};

export function ConsultingReadinessHero({ readiness }: ConsultingReadinessHeroProps) {
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex shrink-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-[10px] text-muted">Indice dossier</p>
            </div>
          </div>
        </div>
        <div className="mt-4">
