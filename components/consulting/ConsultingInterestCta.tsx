    ...
    return (
      <SectionCard title="Richiesta di revisione" padding="sm" tone="primary">
        <div className="space-y-3" data-testid="consultation-confirmation">
          <p className="flex items-center gap-2 text-[13px] font-semibold text-foreground"><CheckCircle2 className="h-4 w-4 text-[var(--success-text)]" />Richiesta ricevuta</p>
          <p className="text-[12px] leading-relaxed text-muted">Stato: {request.status}. Un eventuale contatto avverra secondo la preferenza indicata; non e stata prenotata automaticamente alcuna consulenza.</p>
          <p className="text-[10px] text-muted">Inviata il {new Intl.DateTimeFormat("it-CH").format(new Date(request.createdAt))}.</p>
        </div>
      </SectionCard>
    );
  }
  ...
  return (
    <SectionCard title="Revisione gratuita" padding="sm" tone="primary">
      <form className="space-y-3" onSubmit={(event) => {
        ...
        <p className="text-[12px] leading-relaxed text-muted">Invia il dossier per una verifica umana. La richiesta non implica una raccomandazione, un preventivo o una promessa di risparmio.</p>
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          <button disabled={pending} className="atlas-btn-primary inline-flex flex-1 items-center justify-center gap-2 py-2.5 text-[12px]"><Send className="h-4 w-4" />{pending ? "Invio..." : "Richiedi revisione gratuita"}</button>
        </div>
        {message ? <p role="status" className="text-[11px] text-muted">{message}</p> : null}
        <p className="text-[10px] text-muted">Readiness dossier {readinessPercent}%.</p>
      </form>
    </SectionCard>
  );
}
