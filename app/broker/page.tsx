...
export default async function BrokerDashboard() {
  ...
  return (
    <>
      <OperationsHeader
        eyebrow="Spazio broker"
        title={`Buongiorno, ${data.broker.displayName}`}
        description="Pipeline operativa sulle consulenze assegnate e sulle sole risorse che il cliente ha scelto di condividere."
      />
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <OperationsMetric
          label="Lead attivi"
          value={String(active.length)}
          detail={`${data.leads.length} assegnazioni totali`}
        />
        <OperationsMetric
          label="Da contattare"
          value={String((data.pipeline.assigned ?? 0) + (data.pipeline.submitted ?? 0))}
        />
        <OperationsMetric label="Appuntamenti" value={String(data.appointmentCount)} />
        <OperationsMetric label="In analisi" value={String(data.pipeline.in_review ?? 0)} />
        <OperationsMetric
          label="Conclusi"
          value={String(completed)}
          detail={`${decided ? Math.round((completed / decided) * 100) : 0}% conversione sui casi decisi`}
        />
        <OperationsMetric
          label="Commissioni"
          value={String(data.ledger.length)}
          detail={`${data.contracts.length} contratti`}
        />
      ...
      <div className="mt-6">
        <OperationsPanel
          ...
          action={
            <Link href="/broker/leads" className="text-[12px] font-medium text-accent">
              Tutta la pipeline
            </Link>
          }
        >
              ...
              >
                <div>
                  ...
                  <p className="mt-0.5 text-[11px] text-muted">
                    {consultationTypeLabel(lead.requestType)} · aggiornato {formatDate(lead.updatedAt)}
                  </p>
                </div>
