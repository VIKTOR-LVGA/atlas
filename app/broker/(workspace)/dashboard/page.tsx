...
export const metadata = { title: "Broker Dashboard | ATLAS" };
  ...
  const activity = (
    ...
    .map((event) => {
      ...
      return {
        ...
        at: event.created_at,
        href: `/broker/requests/${event.consultation_request_id}`,
      };
    });
  ...
  return (
    <>
          ...
          <div className="min-w-0">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <PartnerBadge tone="neutral">
                {active.length} richieste ATLAS attive
              </PartnerBadge>
            </div>
          </div>
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <OperationsMetric label="Richieste ATLAS attive" value={String(active.length)} />
        <OperationsMetric
          label="Conversion ATLAS"
          value={`${conversion}%`}
          detail={`${completed} conclusi su ${decided || 0} decisi`}
        />
        <OperationsMetric
          label="Offerte ATLAS aperte"
          value={String(openOffers.length)}
          detail={openPremium > 0 ? `Premio noto ${formatChf(openPremium)}` : undefined}
        />
        <OperationsMetric
          label="Commissioni ATLAS (netto)"
      ...
      <div className="mb-6 grid gap-5 xl:grid-cols-[1.4fr_0.8fr]">
        <section>
          <div className="mb-3 flex items-end justify-between gap-3">
            ...
            <Link href="/broker/requests" className="text-[12px] font-medium text-accent">
              Tutte le richieste
            </Link>
          </div>
          {priorities.length ? (
        ...
        <section>
          ...
          <div className="grid gap-2">
            {[
              { href: "/broker/requests", label: "Apri richieste", area: "leads" as const },
              {
                href: "/broker/appointments",
                label: "Appuntamenti",
                area: "appointments" as const,
              },
              { href: "/broker/offers", label: "Offerte", area: "offers" as const },
              {
                href: "/broker/commissions",
                label: "Commissioni",
                area: "commissions" as const,
              },
              { href: "/broker/analytics", label: "Analytics", area: "analytics" as const },
              { href: "/broker/clients", label: "Clienti", area: "clients" as const },
            ].map((action) => (
              <Link
                key={action.href}
                href={action.href}
        ...
        {stages.some((s) => s.count > 0) ? (
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">
            {stages.map((stage) => (
              <Link
                key={stage.id}
                href={`/broker/requests?status=${stage.id}`}
          ...
          <PartnerEmptyState
            area="leads"
            title="Nessuna richiesta ATLAS attiva"
            description="Quando ATLAS ti assegnerà richieste, qui vedrai la pipeline operativa."
            action={{ href: "/broker/requests", label: "Vai alle richieste ATLAS" }}
          />
        )}
      </section>
      ...
      <div className="mb-6 grid gap-5 xl:grid-cols-2">
        ...
        <OperationsPanel
          ...
          action={
            <Link href="/broker/appointments" className="text-[12px] font-medium text-accent">
              Agenda
            </Link>
          }
        >
